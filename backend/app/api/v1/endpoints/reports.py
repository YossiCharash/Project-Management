...
async def get_project_expense_categories(project_id: int, db: DBSessionDep, user = Depends(require_roles(UserRole.ADMIN, UserRole.PROJECT_MANAGER, UserRole.VIEWER))):
...
async def get_project_transactions(project_id: int, db: DBSessionDep, user = Depends(require_roles(UserRole.ADMIN, UserRole.PROJECT_MANAGER, UserRole.VIEWER))):
